...
export type BuildingKind = "aucune" | "maison" | "villa";
...
function window2D(w: number, h: number, x: number, y: number, z: number) {
...
function steps(width: number, count: number, z: number) {
  const g = new THREE.Group();
  const rise = 0.17;
  const run = 0.3;
...
export function buildBuilding(kind: BuildingKind): THREE.Group | null {
...
export function buildingDepth(kind: BuildingKind) {
