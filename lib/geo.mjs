...
/**
 * Points échelonnés le long de l'orthodromie entre deux lieux, pour tracer la
 * ligne sur la carte.
 *
 * Une simple droite tirée entre deux marqueurs serait fausse : la projection
 * de la carte courbe les grandes distances, et le trait passerait à côté du
 * chemin réellement le plus court. On interpole donc sur la sphère.
 *
 * @param {{lat: number, lon: number}} a
 * @param {{lat: number, lon: number}} b
 * @param {number} [n] nombre de segments
 */
export function orthodromie(a, b, n = 48) {
  const φ1 = rad(a.lat);
  const λ1 = rad(a.lon);
  const φ2 = rad(b.lat);
  const λ2 = rad(b.lon);

  const d = haversine(a, b) / R_TERRE; // angle au centre, en radians
  const deg = 180 / Math.PI;

  // Deux points confondus : il n'y a pas de direction à interpoler.
